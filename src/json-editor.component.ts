...
export class JsonEditorComponent extends AbstractTrackerComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.jsonStoreService.jsonChange
      ...
      .subscribe(json => {
        ...
        if (this.config.tabsConfig) {
          // update tab groups!
          this.tabNameToSubRecordMap = this.tabsUtilService.geTabNameToSubRecordMap(json, this.keyToTabName);
        }
    ...
    if (this.config.tabsConfig) {
      ...
      this.tabNameToSubRecordMap = this.tabsUtilService.geTabNameToSubRecordMap(this._record, this.keyToTabName);
